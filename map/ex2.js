...
function getFoodCategories(foods) {
  const arrays = foods.map(function(type) {
    if (type.isVegetarian === true) {
      return type.food + ' is suitable for vegetarians';
    } else {
      return type.food + ' is not suitable for vegetarians';
    }
  });
  return arrays;
}
